...
const getCategoryBlogsBySlug = async (req, res) => {
  ...
  try {
    ...
    const blogs = await News.find({ categories: category._id }).select(); // Adjust fields as needed
    ...
    const blogsWithCategoryAndService = await Promise.all(blogs.map(async (blog) => {
      ...
      const serviceCategory = await ServiceCategory.findOne({ '_id': blog.servicecategories });
...
const getLatestBlogs = async (req, res) => {
  try {
    // Fetch the latest 5 documents in the 'News' collection
    const blogs = await News.find()
      .sort({ date: -1 }) // Sort by date in descending order
      .limit(5) // Limit the number of results to 5
      .select('title photo slug url'); // Select only the title and photo fields
