...
const getSpecificCategory = async (req, res) => {
  try {
    const { categoryId } = req.query;
    const categories = await ServiceCategory.findById(categoryId);
...
const fetchCategoryUrlPriorityFreq  = async (req, res) => {
  try {
    const categories = await ServiceCategory.find({}, '_id url changeFreq priority lastmod subCategories')
      .populate({
        path: 'subCategories',
...
const fetchCategoryUrlmeta  = async (req, res) => {
  try {
    const categories = await ServiceCategory.find({}, '_id url metatitle metadescription metakeywords metacanonical metalanguage metaschema otherMeta subCategories')
      .populate({
        path: 'subCategories',
...
const fetchCategoryUrlPriorityFreqById = async (req, res) => {
  try {
    ...
    const topCategory = await ServiceCategory.findById(id).select('url priority changeFreq');
...
const fetchCategoryUrlmetaById = async (req, res) => {
  try {
    ...
    categoryData = await ServiceCategory.findById(id).select('url metatitle metadescription metakeywords metalanguage metacanonical metaschema otherMeta');
