...
const insertSubCategory = async (req, res) => {
  ...
  try {
    ...
    categoryDoc.subCategories.push({ category, alt,imgtitle, photo, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
...
const getSpecificCategory = async (req, res) => {
  try {
    const { categoryId } = req.query;
    const categories = await ProductCategory.findById(categoryId);
