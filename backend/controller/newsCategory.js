...
// herosection blogs headings
const getCategoryBySlug = async (req, res) => {
  const { slug } = req.params;
   console.log(slug)
  try {
    // Find the category by slug
    const category = await NewsCategory.findOne({ slug }).select('category details slug _id component'); // Adjust the fields as needed
...
const getCategoryBlogsBySlug = async (req, res) => {
  const { slug } = req.params;
  console.log(slug);

  try {
    // Step 1: Find the category by slug
    const category = await NewsCategory.findOne({ slug }).select('_id category details slug component');
    
    if (!category) {
      return res.status(404).json({ message: 'Category not found' });
    }

    // Step 2: Find all documents in the 'New' collection that belong to this category
    const blogs = await News.find({ categories: category.slug }).select(); // Adjust fields as needed

    // Step 3: Map over the blogs to fetch associated category and service category names
    const blogsWithCategoryAndService = await Promise.all(blogs.map(async (blog) => {
      // Fetch news category name (assuming it might be needed for additional context)
      const categoryName = category ? category.category : 'Uncategorized';

      // Fetch service category name
      const serviceCategory = await ServiceCategory.findOne({ 'slug': blog.servicecategories });
    ...
    res.status(200).json({
      ...
      blogs: blogsWithCategoryAndService
    });
  } catch (error) {
...
const insertCategory = async (req, res) => {
  const { category,details, alt, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq } = req.body;
  ...
  try {
    ...
    const newCategory = new NewsCategory({ category,details, photo, alt, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
...
const insertSubCategory = async (req, res) => {
  ...
  try {
    ...
    categoryDoc.subCategories.push({ category,details, alt, photo, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
...
const insertSubSubCategory = async (req, res) => {
  ...
  try {
    ...
    subCategory.subSubCategory.push({ category,details, photo, alt, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
    await categoryDoc.save();
...
const updateCategory = async (req, res) => {
  ...
  try {
    const updatedCategory = await NewsCategory.findByIdAndUpdate(
      categoryId,
      { category, photo,details, alt, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq },
...
const updatesubsubcategory = async (req, res) => {
  ...
  try {
    ...
    subSubCategory.details = category || subSubCategory.details;
...
const deletecategory = async (req, res) => {
  ...
  try {
    ...
    const hasSubSubcategories = category.subCategories.some(subCat => subCat.subSubCategories.length > 0);
    ...
    // // Validate and construct the path to the category photo
    // const photoPath = category.photo ? path.join(__dirname, '../logos', category.photo) : null;
    // console.log(`Photo path: ${photoPath}`);
    
    // // Only attempt to delete the file if a valid path is provided
...
const deletesubcategory = async (req, res) => {
  ...
  try {
    ...
    // Check if photo exists before attempting to delete it
    if (categoryDoc.photo) {
      const photoPath = path.join(__dirname, '../logos', categoryDoc.photo);
      deleteFile(photoPath); // Only attempt to delete if the photo exists
    }
...
const deletesubsubcategory = async (req, res) => {
  ...
  try {
    ...
    // Only attempt to delete the file if categoryDoc.photo is not null or undefined
    if (categoryDoc.photo) {
      const photoPath = path.join(__dirname, '../logos', categoryDoc.photo);
      deleteFile(photoPath);
    }
    ...
    // Remove reference to the sub-subcategory from all news
    await News.updateMany(
...
const getSpecificCategory = async (req, res) => {
  try {
    ...
    const category = await NewsCategory.findOne(query);
...
const getSpecificSubcategory = async (req, res) => {
  ...
  try {
    ...
    const subCategory = category.subCategories?.find(
      (sub) =>
        sub._id.toString() === subCategoryId || sub.slug === subCategoryId
    );
...
const getSpecificSubSubcategory = async (req, res) => {
  ...
  try {
    ...
    const subSubCategory = subCategory.subSubCategories?.find(
      (subSub) =>
        subSub._id.toString() === subSubCategoryId || subSub.slug === subSubCategoryId
    );
    if (!subSubCategory)
      return res.status(404).json({ message: "Sub-subcategory not found" });
