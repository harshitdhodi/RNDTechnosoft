...
exports.getLeftStaff = async (req, res) => {
    try {
...
exports.getRightStaff = async (req, res) => {
    try {
        // Set the limit to 10 for the first 10 staff photos
        const limit = 3;
        ...
        const ourstaff = await OurStaff.find()
            .select('photo') // Select only the 'photo' field
            .sort({ S_id: -1 }) // Sort by createdAt in descending order
            .limit(limit); // Limit to 3 documents
