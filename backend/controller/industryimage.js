const IndustryImage = require('../model/industryimage'); // Assuming this is your model path
const IndustryCategory = require('../model/industriescategory'); // Ensure you have the ServiceCategory model imported
...
exports.getAllImages = async (req, res) => {
    try {
        ...
        const images = await IndustryImage.find(filter).populate('categoryId', 'category');
...
exports.getAllSubImages = async (req, res) => {
    try {
        ...
        const images = await IndustryImage.find(filter)
            .populate({
...
exports.addNewImage = async (req, res) => {
    try {
        ...
        const category = await IndustryCategory.findById(categoryId);
        if (!category) {
       ...
       const  headingType = 'main'; // Mark this as a sub-sub-service
        ...
        const newImage = new IndustryImage({ images: image, alt,imgtitle, categoryId, photoType, slug ,headingType});
        await newImage.save();
...
exports.addNewSubImage = async (req, res) => {
    try {
        ...
        const newImage = new IndustryImage({
            images: image,
            alt,
            imgtitle,
...
exports.deleteImage = async (req, res) => {
    try {
        ...
        const image = await IndustryImage.findById(id);
        ...
        await IndustryImage.findByIdAndDelete(id);
...
exports.getSingleImage = async (req, res) => {
    ...
    try {
        const image = await IndustryImage.findById(id).populate('categoryId', 'category'); // Populate to get category names
...
exports.getAllPackagesFront = async (req, res) => {
    try {
      ...
      for (const category of categories) {
        ...
        const images = await IndustryImage.find({
  ...
  exports.getAllCompanyImages = async (req, res) => {
    try {
      ...
      const companyImages = await IndustryImage.find({
        photoType: 'company'
      });
...
exports.updateImage = async (req, res) => {
    try {
        ...
        const updatedImage = await IndustryImage.findByIdAndUpdate(
            id,
            updateData,
            { new: true }
        );
...
exports.getGalleryById = async (req, res) => {
    ...
    try {
        ...
        const gallery = await IndustryImage.findOne({
            _id: id,
