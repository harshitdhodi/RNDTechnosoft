...
const getContentTypeByType = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error retrieving contents by type", error });
  }
};
...
const toggleStatus = async (req, res) => {
  ...
  try {
    ...
    if (!existingContent) {
      return res
        .status(404)
        .json({ message: "Content not found for the given ID" });
    }
...
const addSubsection = async (req, res) => {
  ...
  try {
    ...
    existingContent.subsections.push({
      title,
      description,
      photo,
      photoAlt,
  ...
  try {
    ...
    if (req.body.title && req.body.description) {
      const subsection = {
        ...
        serviceparentCategoryId: req.body.serviceparentCategoryId,
        servicesubCategoryId: req.body.servicesubCategoryId,
        servicesubSubCategoryId: req.body.servicesubSubCategoryId,
      };
    ...
    return res.status(200).json({
      message: "Subsections updated successfully",
      content: existingContent,
    });
  } catch (error) {
...
const updateSubsectionsId = async (req, res) => {
  ...
  try {
    ...
    if (
      subsectionIndex < 0 ||
      subsectionIndex >= existingContent.subsections.length
    ) {
    ...
    if (req.body.description)
      subsectionToUpdate.description = req.body.description;
...
const deleteQuestion = async (req, res) => {
  ...
  try {
    ...
    if (
      questionIndex < 0 ||
      questionIndex >= existingContent.questions.length
    ) {
...
const deletesubsection = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Subsection and photo deleted successfully" });
  } catch (error) {
...
const getToolsByServiceSlug = async (req, res) => {
  try {
    const { slug } = req.query;

    // Fetch content based on the content type
    const content = await Content.findOne({
      contentType: "weareexpertsin",
    }).lean();

    // If slug is undefined, return the full content
    if (slug === "undefined") {
      return res.status(200).json({
        data: content,
      });
    ...
    // Step 1: First check if any subsections match the given slug
    const directMatch = {
      ...content,
      subsections: content.subsections.filter(
        (subsection) =>
          subsection.serviceparentCategoryId === slug ||
          subsection.servicesubCategoryId === slug ||
          subsection.servicesubSubCategoryId === slug
      ),
    };

    if (directMatch.subsections.length > 0) {
      return res.status(200).json({
        data: directMatch,
      });
    }

    // Step 2: Find the service category and determine the hierarchy
    const serviceCategory = await ServiceCategory.findOne({
      $or: [
        { slug: slug }, // Check main category
        { "subCategories.slug": slug }, // Check subcategories
        { "subCategories.subSubCategory.slug": slug } // Check sub-subcategories
      ]
    }).lean().exec();
    ...
    // Step 3: Find the matching category and its parent slugs
    const findCategoryHierarchy = (serviceCategory, targetSlug) => {
      // Check if it's a main category
      if (serviceCategory.slug === targetSlug) {
        return {
          mainSlug: serviceCategory.slug,
          subSlug: null,
          subSubSlug: null,
        };
      }
      ...
      for (const subCategory of serviceCategory.subCategories) {
    ...
    const hierarchy = findCategoryHierarchy(serviceCategory, slug);
    ...
    // Step 4: Filter subsections based on the hierarchy
    const filteredSubsections = {
      ...content,
      subsections: content.subsections.filter((subsection) => {
        // For main category
        if (hierarchy.mainSlug === slug) {
          return subsection.serviceparentCategoryId === hierarchy.mainSlug;
        }
        
        // For subcategory
        if (hierarchy.subSlug === slug) {
          return (
            subsection.servicesubCategoryId === hierarchy.subSlug ||
            subsection.serviceparentCategoryId === hierarchy.mainSlug
          );
        }

        // For sub-subcategory
        if (hierarchy.subSubSlug === slug) {
          return (
            subsection.servicesubSubCategoryId === hierarchy.subSubSlug ||
            subsection.servicesubCategoryId === hierarchy.subSlug ||
            subsection.serviceparentCategoryId === hierarchy.mainSlug
          );
        }
    ...
    return res.status(200).json({
      ...
      hierarchy, // Optionally include hierarchy information
    });
