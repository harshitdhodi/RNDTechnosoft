...
const TestimonialsTable = ({ categoryId,subcategoryId,subsubcategoryId }) => {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ value }) => {
          ...
          return firstImage ? <img src={`/api/image/download/${firstImage}`} alt="Testimonial" className="w-32 h-20 object-cover" /> : null;
        },
        disableSortBy: true,
      },
      {
  ...
  const fetchData = async (pageIndex) => {
    setLoading(true);
    try {
        const response = await axios.get(`/api/testimonial/subsub/getTestimonial`, {
            params: {
              page: pageIndex + 1,
  ...
  const deleteTestimonial = async (id) => {
    try {
      const response = await axios.delete(`/api/testimonial/deleteTestimonial?id=${id}`, { withCredentials: true });
    ...
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <Link to={`/testimonials/createTestimonials/${categoryId}/${subcategoryId}/${subsubcategoryId}`}><FaPlus size={15} /></Link>
        </button>
      </div>
      <div className="mb-4">
        <input
