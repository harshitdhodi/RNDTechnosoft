...
const DesignProcess = ({ categoryId,subcategoryId ,subsubcategoryId}) => {
 ...
 className="text-blue-500 hover:text-blue-700 transition">
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/api/designProcess/subsub/category`, {
        params: {
  ...
  return (
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to={`/services/designProcess/${categoryId}/${subcategoryId}/${subsubcategoryId}`} className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <FaPlus size={15} />
        </Link>
      </div>
