...
const TechnologySecDataForm = () => {
    ...
    const [technologies, setTechnologies] = useState([]); // Store technology options
    ...
    // Debounced ReactQuill change handler
    ...
    const handleQuillChange = debounce((field, value, index) => {
        ...
        } else if (field === 'card.subHeading') {
            const updatedCards = [...formData.card];
            updatedCards[index] = { ...updatedCards[index], subHeading: value };
            setFormData({ ...formData, card: updatedCards });
        }
    }, 300);
        ...
        try {
            ...
            formData.card.forEach((card, index) => {
                if (files[index]) {
                    ...
                    fileFormData.append(`card[${index}][photo]`, '');
                }
