...
const Inquiry = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/popupinquiry/getpopupinquiries`, { withCredentials: true });
  ...
  const deleteInquiry = async (id) => {
    try {
      await axios.delete(`/api/popupinquiry/deletePopupInquiries?id=${id}`, { withCredentials: true });
      fetchData();
    } catch (error) {
      console.error(error);
    }
