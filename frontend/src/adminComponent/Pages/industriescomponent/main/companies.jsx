      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <span
            ...
            onClick={() => navigate(`/industries/editImages/${categoryId}?id=${row.original._id}&photoType=${photoType}`)}
          >
  ...
  const fetchData = async (categoryId) => {
    ...
    try {
      const response = await axios.get(
        `/api/industryImages/getGallery?categoryId=${categoryId}&photoType=${photoType}`,
    ...
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <Link
            to={`/industries/createImage/${categoryId}?photoType=${photoType}`}
          >
            <FaPlus size={15} />
          </Link>
        </button>
