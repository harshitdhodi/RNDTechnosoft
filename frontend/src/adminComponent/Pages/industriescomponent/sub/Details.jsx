...
const IndustriessTable = ({ categoryId, subcategoryId }) => {
    ...
    () => [
      {
        ...
        Cell: ({ row }) => (
          <span
            ...
            onClick={() => navigate(`/industries/editSubIndustries/${row.original._id}`)}
          >
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="flex gap-4">
            ...
            <Link to={`/industries/editSubIndustries/${row.original._id}`} className="text-blue-500 hover:text-blue-700 transition">
              <FaEdit />
            </Link>
  ...
  const fetchData = async (pageIndex) => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/industiesDetails/getSubIndustriesDetails?subcategoryId=${subcategoryId}&page=${pageIndex + 1}`, { withCredentials: true });
  ...
  return (
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to={`/industries/createIndustries/${categoryId}/${subcategoryId}`} className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <FaPlus size={15} />
        </Link>
      </div>
      <div className="mb-4">
