...
const CategoryTable = () => {
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="flex items-center gap-2 hover:text-blue-500 cursor-pointer"
            onClick={() => navigate(`/PackageCategory/editPackageCategory/${row.original.slug}`)}>
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/packages/getall`, { withCredentials: true });
  ...
  const deleteCategory = async ({ id, categoryId, subCategoryId, subSubCategoryId }) => {
    ...
    if (categoryId && subCategoryId && subSubCategoryId) {
      url = `/api/packages/deletesubsubcategory?categoryId=${categoryId}&subCategoryId=${subCategoryId}&subSubCategoryId=${subSubCategoryId}`;
    } else if (categoryId && subCategoryId) {
      url = `/api/packages/deletesubcategory?categoryId=${categoryId}&subCategoryId=${subCategoryId}`;
    } else {
      url = `/api/packages/deletecategory?id=${id}`;
    }
