...
const BuildInfo = () => {
  ...
  useEffect(() => {
    const fetchBuildData = async () => {
      ...
      try {
        ...
        console.log('Build data:', response.data);
        ...
        if (Array.isArray(data)) {
          applicationsData = data.find(item => item.type === 'build');
        } else {
          applicationsData = data.type === 'applications' ? data : null;
        }
        
        setBuildData(applicationsData);
      ...
      <div className="flex flex-col items-center justify-center py-20 px-4">
        <div className="text-center">
          ...
          <button 
            ...
            className="btn-yellow rounded-lg text-gray-700 transition"
          >
            Back to Industries
          </button>
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">The requested information could not be found.</p>
          <button 
            onClick={() => navigate('/industries')}
  ...
  const title = buildData?.heading || buildData?.title || buildData?.name;
  const description = buildData?.subHeading || buildData?.description || buildData?.content;
  const buttonText = buildData?.ctaText;
