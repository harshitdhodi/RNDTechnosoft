...
const ProjectsSection = () => {
  ...
  const fetchCategories = async () => {
    try {
      setPortfolios([]);
      const response = await axios.get(`/api/Portfolio/getAllCategory`);
  ...
  const fetchPortfolios = async () => {
    ...
    try {
      ...
      if (selectedCategory.slug === "all") {
        response = await axios.get(`/api/Portfolio/getPortfolioFront`);
      } else {
        response = await axios.get(
          `/api/Portfolio/getCategoryPortfolio?categoryId=${selectedCategory.slug}`
        );
      }
      const fetchedPortfolios = response.data?.data || response.data;
